...
import '../utils/amount_formatter.dart' show formatAmount, formatExchangeRate;
import '../utils/date_formatter.dart';
import '../utils/language_notifier.dart';
import '../widgets/converter/currency_row.dart';
import '../widgets/converter/divider_line.dart';
import '../widgets/converter/keypad.dart';
import '../widgets/converter/rate_panel.dart';
import '../widgets/converter/status_time.dart';
import '../widgets/history/history_chart_bottom_sheet.dart';
import '../widgets/settings/settings_bottom_sheet.dart';
import 'currency_picker_page.dart';
import 'developer_website_dialog.dart';
...
class _CurrencyConverterScreenState extends State<CurrencyConverterScreen> {
  ...
  String _formatRateText() {
    ...
    return '1 $_fromCurrency = ${formatExchangeRate(rate)} $_toCurrency';
  }
  ...
  void _recalculateLinkedValue() {
    try {
      ...
      if (rate == null || rate.isNaN || rate.isInfinite) {
        ...
        final result = topValue * rate;
        if (result.isNaN || result.isInfinite) {
          _bottomDisplay = '0';
        } else {
          _bottomDisplay = formatAmount(result);
        }
      }
    } catch (_) {
    ...
    showModalBottomSheet(
      ...
      builder: (sheetContext) {
        return SettingsBottomSheet(
          ...
          onDeveloperWebsiteTap: () async {
            Navigator.of(sheetContext).pop();
            await showDeveloperWebsiteDialog(context);
          },
        );
      },
    );
